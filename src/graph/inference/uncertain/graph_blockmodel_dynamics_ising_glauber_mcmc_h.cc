...
GEN_DISPATCH(mcmc_ising_glauber_state, MCMC<State>::template MCMCPseudoIsingState,
             MCMC_PSEUDO_ISING_STATE_params(State))

python::object mcmc_ising_glauber_sweep_h(python::object omcmc_state,
        ...
        dynamics_state<state_t>::dispatch
            (odynamics_state,
             [&](auto& ls)
             {
                 ...
                 mcmc_ising_glauber_state<dynamics_state_t>::make_dispatch
                     (omcmc_state,
                      [&](auto& s)
                      {
...
([]
{
    ...
    def("mcmc_ising_glauber_sweep_h", &mcmc_ising_glauber_sweep_h);
});
