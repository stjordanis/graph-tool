...
GEN_DISPATCH(mcmc_pseudo_cising_state, MCMC<State>::template MCMCPseudoIsingState,
             MCMC_PSEUDO_ISING_STATE_params(State))

python::object mcmc_pseudo_cising_sweep_h(python::object omcmc_state,
        ...
        dynamics_state<state_t>::dispatch
            (odynamics_state,
             [&](auto& ls)
             {
                 ...
                 mcmc_pseudo_cising_state<dynamics_state_t>::make_dispatch
                     (omcmc_state,
                      [&](auto& s)
                      {
...
void export_pseudo_cising_mcmc_h()
{
    using namespace boost::python;
    def("mcmc_pseudo_cising_sweep_h", &mcmc_pseudo_cising_sweep_h);
}
