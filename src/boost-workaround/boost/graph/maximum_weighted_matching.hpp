...
namespace boost
{
    ...
    public:
        ...
        typedef typename detail::VERTEX_STATE vertex_state_t;
        typedef typename graph_traits<Graph>::vertex_iterator vertex_iterator_t;
        typedef typename graph_traits<Graph>::vertex_descriptor vertex_descriptor_t;
        ...
        weighted_augmenting_path_finder(const Graph& arg_g, WeightMap arg_weight, MateMap arg_mate, VertexIndexMap arg_vm) :
        g(arg_g),
        weight(arg_weight),
        vm(arg_vm),
        ...
        old_label(old_label_vector.begin(), vm)
        {
            ...
            min_abs = std::numeric_limits<edge_property_t>::max();
        ...
        bool is_zero(T x)
        {
            return is_zero_dispatch(x, typename std::is_floating_point<T>::type());
        }

        template <class T>
        bool is_zero_dispatch(T x, std::true_type)
        {
            return std::abs(x) < min_abs * std::sqrt(2 * std::numeric_limits<T>::epsilon());
        }

        template <class T>
        bool is_zero_dispatch(T x, std::false_type)
        {
            return x == 0;
        }
        ...
        std::pair<vertex_descriptor_t, vertex_descriptor_t> missing_label(vertex_descriptor_t b_base)
        {
            ...
            std::pair<vertex_descriptor_t, vertex_state_t> child(outlet[b_base], detail::V_EVEN);
        ...
        inline edge_property_t slack(const edge_descriptor_t& e) const
        {
            ...
            v = source(e, g);
            w = target(e, g);
            return dual_var[v] + dual_var[w] - 4*get(weight, e);
        }
                                                              ...
                                                              bool use_old = false) const
        {
            if (v.second == detail::V_EVEN)
            {
                ...
                return std::make_pair(label_S[v.first], detail::V_ODD);
            }
            else if (v.second == detail::V_ODD)
            {
                vertex_descriptor_t w = use_old ? old_label[v.first].first : base_vertex(label_T[v.first]);
                return std::make_pair(w, detail::V_EVEN);
            }
            return std::make_pair(v.first, detail::V_UNREACHED);
        }
        ...
        {
            std::pair<vertex_descriptor_t, vertex_state_t> v_up(v, detail::V_EVEN);
            std::pair<vertex_descriptor_t, vertex_state_t> w_up(w, detail::V_EVEN);
        ...
        {
            std::pair<vertex_descriptor_t, vertex_state_t> u(v_prime, detail::V_ODD);
            std::pair<vertex_descriptor_t, vertex_state_t> u_up(w_prime, detail::V_EVEN);

            for (; u_up.first != stop_vertex; u = u_up, u_up = parent(u))
            {
                if (u_up.second == detail::V_EVEN)
                {
                    if (!in_top_blossom(u_up.first)->sub_blossoms.empty())
                        outlet[u_up.first] = label_T[u.first];
                    label_T[u_up.first] = outlet[u.first];
                }
                else if (u_up.second == detail::V_ODD)
        ...
        void retrieve_augmenting_path(vertex_descriptor_t v, vertex_descriptor_t w, vertex_state_t v_state)
        {
            ...
            else if (v_state == detail::V_EVEN)
            {
                aug_path.push_back(v);
                retrieve_augmenting_path(label_S[v], w, detail::V_ODD);
            }
            else if (v_state == detail::V_ODD)
            {
                if (outlet[v] == v)
                    aug_path.push_back(v);
                else
                    reversed_retrieve_augmenting_path(outlet[v], v, detail::V_EVEN);
                retrieve_augmenting_path(label_T[v], w, detail::V_EVEN);
            }
        }
        ...
        void reversed_retrieve_augmenting_path(vertex_descriptor_t v, vertex_descriptor_t w, vertex_state_t v_state)
        {
            ...
            {
                reversed_retrieve_augmenting_path(label_S[v], w, detail::V_ODD);
                aug_path.push_back(v);
            }
            else if (v_state == detail::V_ODD)
            {
                reversed_retrieve_augmenting_path(label_T[v], w, detail::V_EVEN);
                if (outlet[v] != v)
                    retrieve_augmenting_path(outlet[v], v, detail::V_EVEN);
                else
                    aug_path.push_back(v);
            }
        }
        ...
        void relabel(vertex_descriptor_t v)
        {
            ...
            { // if v is a new base vertex
                std::pair<vertex_descriptor_t, vertex_state_t> u(v, detail::V_EVEN);
                ...
                { // if old base is exposed then new label_T[v] is not in b,
                    ...
                    if (b != blossom_ptr_t())
                    {
                        u = std::make_pair(b->get_base(), detail::V_ODD);
            ...
            { // if v is an old base vertex
                ...
                std::pair<vertex_descriptor_t, vertex_state_t> u(b->get_base(), detail::V_ODD);
                        ...
                        vertex_descriptor_t w, vertex_descriptor_t w_free_ancestor)
        {
            ...
            reversed_retrieve_augmenting_path(v, v_free_ancestor, detail::V_EVEN);
            retrieve_augmenting_path(w, w_free_ancestor, detail::V_EVEN);
        ...
        bool augment_matching()
        {
            ...
            while (!even_edges.empty() && !found_alternating_path)
            {
                ...
                {
                    if (!is_zero(slack(current_edge)) && slack(current_edge) > 0)
                    {
                    ...
                    else if (is_zero(slack(current_edge)))
                    {
        ...
        bool adjust_dual()
        {
            ...
            for (boost::tie(vi,vi_end) = vertices(g); vi != vi_end; ++vi)
            {
                ...
                if (*vi == base_vertex(*vi))
                    delta3 = std::min(delta3, edge_property_t(gamma[*vi]/2));
            }

            for (blossom_iterator_t bi = top_blossoms.begin(); bi != top_blossoms.end(); ++bi)
            {
                vertex_descriptor_t b_base = (*bi)->get_base();
                if (label_T[b_base] != graph_traits<Graph>::null_vertex() && is_zero(pi[b_base]))
                    delta2 = std::min(delta2, edge_property_t((*bi)->dual_var/2));
            }
            ...
            for (boost::tie(vi,vi_end) = vertices(g); vi != vi_end; ++vi)
            {
                ...
                if (label_S[v_prime] != graph_traits<Graph>::null_vertex())
                    dual_var[v] -= delta;
                else if (label_T[v_prime] != graph_traits<Graph>::null_vertex() && is_zero(pi[v_prime]))
                    dual_var[v] += delta;
            ...
            for (boost::tie(vi,vi_end) = vertices(g); vi != vi_end; ++vi)
            {
                ...
                if (label_T[v] != graph_traits<Graph>::null_vertex() && is_zero(pi[v]))
            ...
            if (delta == delta2 && delta != delta3)
            {
                ...
                for (blossom_iterator_t bi = top_blossoms.begin(); bi != top_blossoms.end();)
                {
                    ...
                    vertex_descriptor_t b_base = b->get_base();
                    if (is_zero(b->dual_var) && label_T[b_base] != graph_traits<Graph>::null_vertex() && is_zero(pi[b_base]))
            ...
            while (true)
            {
                ...
                for (vertex_vec_iter_t i = base_nodes.begin(); i != base_nodes.end(); ++i)
                {
                    if (is_zero(gamma[*i]))
                    {
                        ...
                        {
                            if (critical_edge[*i][*j] != null_edge && is_zero(slack(critical_edge[*i][*j].first)))
    ...
    private:
        ...
        edge_property_t min_abs;
    };

    template <typename Graph, typename WeightMap, typename MateMap, typename VertexIndexMap>
    void maximum_weighted_matching(const Graph& g, WeightMap weight, MateMap mate, VertexIndexMap vm)
    {
        empty_matching<Graph, MateMap>::find_matching(g, mate);
        weighted_augmenting_path_finder<Graph, WeightMap, MateMap, VertexIndexMap> augmentor(g, weight, mate, vm);
    ...
    template <typename Graph, typename WeightMap, typename MateMap>
    inline void maximum_weighted_matching(const Graph& g, WeightMap weight,
                                          MateMap mate)
    {
        maximum_weighted_matching(g, weight, mate, get(vertex_index, g));
    }
    ...
    private:
        ...
        void select_edge(edge_iterator_t ei)
        {
            if (ei == ei_end)
            {
                if (matching_weight_sum(g, weight, mate) > matching_weight_sum(g, weight, best_mate))
                {
                    vertex_iterator_t vi, vi_end;
                    for (boost::tie(vi,vi_end) = vertices(g); vi != vi_end; ++vi)
